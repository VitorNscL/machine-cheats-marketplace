...
window.pages.chat = async function chatPage(me) {
  ...
  if (m.authorRole === 'ADMIN') {
    badges.push('<span class="chat-badge admin">ADMIN</span>');
    } else if (m.authorIsVip) {
    badges.push('<span class="chat-badge vip">VIP</span>');
    }

const badgeHtml = badges.length ? ' ' + badges.join(' ') : '';

  // ✅ Botão apagar (mantém sua lógica)
  const delBtn =
    canMod && !isDeleted
      ? `<button class="btn-neon" data-del="${m.id}" style="padding:4px 8px; font-size:12px">Apagar</button>`
      : '';
  ...
  return `
    <div class="row" style="align-items:flex-start; margin-bottom:10px" data-msg="${m.id}">
      <div style="flex:1">
        <div class="help">
          <strong>@${window.api.escapeHtml(m.authorNick)}</strong>${badgeHtml}
          <span class="muted"> • </span>${date}
        </div>
